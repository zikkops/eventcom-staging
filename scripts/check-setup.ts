...
async function main() {
  ...
  if (!process.env.DATABASE_URL) {
    ...
    try {
      ...
      // How many events still point at photos in public/ rather than ImageKit?
      if (tables.some((table) => table.tablename === "events")) {
        const [content] = await sql`SELECT
          (SELECT count(*)::int FROM events) AS events,
          (SELECT count(*)::int FROM events
            WHERE card_image LIKE '/%' OR hero_image LIKE '/%'
              OR album::text LIKE '%"/%' OR films::text LIKE '%"/%') AS local`;
        console.log(
          `            ${content.events} events; ${
            Number(content.local) === 0 ? "every photo is on ImageKit" : `${content.local} still using public/`
          }`,
        );
      }
    } catch (error) {
